...
function applyCustomStyling(windowURL) {
            ...
            fightContainer.innerHTML =
              ...
              '<div class="dgt-fight-simulation" eta="' + i + '">' +
                '<table class="dgt-fight-simulator-by-rof">' +
                  ...
                  '</tr>' +
                  '<tr class="top-header">' +
                    '<th colspan="5">Resource Units Lost [1.5 Metal = 1 Mineral = 1 Unit]</th>' +
                  '</tr>' +
  ...
  // -- alliances rankings
  if (windowURL[1] === 'rankings' && (windowURL.length >= 4 && windowURL[2] === 'alliances')) {
    document.querySelectorAll('span.allied').forEach((allied => {
      allied.parentElement.parentElement.classList.add('allied-bg', 'rank-row');
    }));
    ...
    document.querySelector('.rankingsList').classList.add('alliancesRankingsList');
